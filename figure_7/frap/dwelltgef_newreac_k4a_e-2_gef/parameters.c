...
//0 k1a_uM = 10;       %s-1,       BemGEFdc -> BemGEFd
a[subv][11] = p.kmi[0]*state[subv][8];	

//1 k1b_uM = 10;       %s-1,       BemGEF -> BemGEFc
a[subv][1] = p.kmi[1]*state[subv][2];	
//1 k1b_uM = 10;       %s-1,       BemGEFd -> BemGEFdc
a[subv][12] = p.kmi[1]*state[subv][7];	
	
//2 k2a_uM = 0.16;     %uM-1.s-1,  BemGEF + Cdc42D -> Cdc42T + BemGEF
a[subv][2] = kMesoHetero(2,2,3,subv,state,p)*state[subv][2]*state[subv][3];
//2 k2a_uM = 0.16;     %uM-1.s-1,  BemGEFd + Cdc42D -> Cdc42T + BemGEFd
a[subv][13] = kMesoHetero(2,7,3,subv,state,p)*state[subv][7]*state[subv][3];
...
//4 k3_uM  = 0.35;     %uM-1.s-1,  BemGEFd42 + Cdc42D -> Cdc42T + BemGEFd42
a[subv][14] = kMesoHetero(4,6,3,subv,state,p)*state[subv][6]*state[subv][3];	
	
//5 k4a_uM = 10;       %uM-1.s-1,  BemGEF + Cdc42T -> BemGEF42
a[subv][5] = kMesoHetero(5,2,0,subv,state,p)*state[subv][2]*state[subv][0];		
//5 k4a_uM = 10;       %uM-1.s-1,  BemGEFd + Cdc42T -> BemGEFd42
a[subv][15] = kMesoHetero(5,7,0,subv,state,p)*state[subv][7]*state[subv][0];		
...
//6 k4b_uM = 10;       %s-1,       BemGEFd42 -> BemGEFd + Cdc42T
a[subv][16] = p.kmi[5] > 0 ? p.kmi[6]*kMesoHetero(5,7,0,subv,state,p)*p.h*p.h/p.kmi[5]*state[subv][6] : p.kmi[6]*state[subv][6];
...
//9 k7_uM  = 10;       %uM-1.s-1,  BemGEFdc + Cdc42T -> BemGEFd42
a[subv][17] = kMesoHetero(9,8,0,subv,state,p)*state[subv][0]*state[subv][8];		

//10 k8_uM  = ;       %uM-1.s-1,  Cdc42c + BemGEF42 -> Cdc42T + BemGEF42 
a[subv][10] = kMesoHetero(10,4,1,subv,state,p)*state[subv][4]*state[subv][1];		
//10 k8_uM  = ;       %uM-1.s-1,  Cdc42c + BemGEFd42 -> Cdc42T + BemGEFd42 
a[subv][18] = kMesoHetero(10,4,6,subv,state,p)*state[subv][4]*state[subv][6];		
...
switch(reaction){
	...
	//BemGEF -> BemGEFc	
		...
		break;
	case 12:
	//BemGEFd -> BemGEFc   #different from frap	
		state[subvnext][7]--;
		state[subvnext][5]++;
		break;
	...
	//BemGEF42 + Cdc42D -> Cdc42T + BemGEF42	
		...
		break;
	case 14:
	//BemGEFd42 + Cdc42D -> Cdc42T + BemGEFd42	
		state[subvnext][3]--;
		state[subvnext][0]++;	
		break;
	...
	//BemGEF42 + Cdc42c -> Cdc42T + BemGEF42
		...
		break;	
	case 18:
	//BemGEFd42 + Cdc42c -> Cdc42T + BemGEFd42
		state[subvnext][4]--;
		state[subvnext][0]++;
		break;	
