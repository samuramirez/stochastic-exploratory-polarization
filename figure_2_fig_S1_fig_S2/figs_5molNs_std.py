...
protr=0.005
L=1.0
nsims=10000 #10000 for nprot 5
Ns=[40,60,20]

number = len(Ns)
#cmap = plt.get_cmap('jet')
#colors = [cmap(i) for i in np.linspace(0, 1, number)]
#colors = [cm.hot(i) for i in np.linspace(0.7, 0, number)]
colors = [  cm.jet(0.5) ,cm.jet(0.9), cm.jet(0.2) ]
#linestyles=[(0,(1,0.5)),'dashed','solid']
linestyles=['solid','solid',(0,(1,0.5))]
timeTS=0
...
for imet,met in enumerate(mets):  
    ...
    for iN, N in enumerate(Ns):
        ...
        if os.path.isfile(file):
            ...
            plt.plot(tSG,stdAtSG,label=r'$h =L/$'+ '{:.0f}'.format(N)+'='+'{:.1f}'.format(L/N/protr)+r'$\rho$'  , alpha=1,color=colors[iN],linestyle=linestyles[iN])
            ...
            #histSG=np.ravel(Ats[:,9000:9999]) 
            #plt.hist(histSG,bins,density=True, color=colors[iN],alpha=1,histtype='step',linewidth=1.5)
    ...
    #plt.plot(timeTS,meanAt,label='particle-based', alpha=1, color='black', linestyle=(0,(5,2)))
    plt.plot(timeTS,stdAtPB,label='particle-based', alpha=1, color='black', linestyle=(0,(5,2)))
