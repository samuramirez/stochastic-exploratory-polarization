...
//if(n==0){n=1;}
//this is to estimate a reversible rate accounting for diffusion in the voxel when there are no association reactants but there are complexes that can dissociate
	
//free area equality	
//pi*bsr^2 - pi*sigma^2 = 2*h*h/n - pi*sigma^2 
...
if (kmeso > p.kmi[reaction] ){printf("kmeso = %f, kmi = %f , lambda = %f, reaction %d\n", kmeso, p.kmi[reaction], lambda, reaction); exit(1);}
...
//	randnum = (randnum - rate[subvnext][0]/rate[subvnext][2])/(1 - rate[subvnext][0]/rate[subvnext][2]);
	...
	while(randnum==0.0 || randnum==1.00){randnum= (double)rand()/RAND_MAX;}
	sum_Drates=0;
