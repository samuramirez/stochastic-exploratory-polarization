...
sim_dir = './reversiblePB/kD' + kaDPB[kaDind] + 'kd' + kdPB[kdind] + 'n' + str(nprot) + 'std'
...
for imet,met in enumerate(mets):  
    ...
    for iN, N in enumerate(Ns):
        filename= 'rev' + met + 'N' + str(N) + 'ka' + kaDSG[kaDind] + 'D' + 'kd' + kdSG[kdind] + 'nA0' + str(nprot)+'nB0'+str(nprot)+'std.dat'
        ...
        if os.path.isfile(file):
            ...
            plt.plot(tSG,stdAtSG,label=r'$h =L/$'+ '{:.0f}'.format(N)+'='+'{:.1f}'.format(L/N/protr)+r'$\rho$'  , alpha=0.7,color=colors[iN],linestyle=linestyles[iN])
    ...
    plt.plot(timeTS,stdAtPB,label='particle-based', alpha=0.7, color='black', linestyle='dashed')
    #plt.ylim(0,0.09)
    plt.xscale('log')
...
#axarr[figrow,figcol].set_title('kaD' + kaDSG[kaDind] + '_kd_' + kdSG[kdind] + '_N_' + N)
#axarr[0,0].legend()
